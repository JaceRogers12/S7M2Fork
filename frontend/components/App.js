...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (editing) {
    let member = members.find(member=> member.id === editing)
    setFormValues({
      id: member.id,
      fname: member.fname,
      lname: member.lname,
  ...
  const editExistingMember = () => {
    ...
    const {id, fname, lname, bio} = formValues;
      ...
      <div id="membersForm">
        ...
        <form onSubmit={onSubmit}>
          <div>
            ...
            <input id="fname" type="text" value={formValues.fname} onChange={event => onChange(event)} placeholder="Type First Name" />
          </div>

          <div>
            <label htmlFor="lname">Last Name </label>
            <input id="lname" type="text" value={formValues.lname} onChange={event => onChange(event)} placeholder="Type Last Name" />
          </div>

          <div>
            <label htmlFor="bio">Bio </label>
            <textarea id="bio" value={formValues.bio} onChange={event => onChange(event)} placeholder="Type Bio" />
          </div>
